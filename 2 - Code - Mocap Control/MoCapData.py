...
class MarkerSetData:
    def __init__(self):
        self.marker_data_list=[] #List of Mocap data, the contents of which we will send to the drone
...
class LegacyMarkerData:
    ...
    def get_as_string(self, tab_str="  ", level=0):
        ...
        for i in range(marker_count):
            pos = self.marker_pos_list[i]
            # The line below shows how the points lie in the array, this is what we will send to the drone for position, still working on altitude
            out_str+="%sMarker %3.1d pos : [x=%3.2f,y=%3.2f,z=%3.2f]\n"%(out_tab_str2,i,pos[0], pos[1], pos[2])
        return out_str
